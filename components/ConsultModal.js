  ...
  return (
    <div className="consultModal__backdrop" onClick={safeClose}>
      <div className="consultModal" onClick={(e) => e.stopPropagation()}>
        <button className="consultModal__close" onClick={safeClose} aria-label="Закрыть">×</button>
        ...
        <form className="consultModal__form" onSubmit={(e) => { e.preventDefault(); safeClose(); }}>
      ...
      <style jsx>{`
        .consultModal__backdrop{position:fixed;inset:0;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;z-index:9999}
        .consultModal{position:relative;width:min(560px,92vw);background:#fff;border-radius:18px;padding:20px;box-shadow:0 24px 80px rgba(0,0,0,.25)}
        .consultModal__close{position:absolute;right:14px;top:8px;border:0;background:none;font-size:28px;cursor:pointer;line-height:1}
        ...
        .consultModal__input{width:100%;border:1px solid #e8e8e8;border-radius:12px;padding:12px 14px;outline:none}
      `}</style>
    </div>
  );
}
